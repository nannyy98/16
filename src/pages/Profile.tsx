...
import {
  ...
  Package, Pencil, X, Phone, MapPin, Clock, Sun, Moon, Monitor, MessageCircle, Sparkles, Wallet, Users,
} from 'lucide-react';
...
import { useOrders, useUserReferrals, useUserProfile, useUpdateProfile, useWalletStats } from '../lib/supabase/hooks';
...
export const Profile = () => {
  ...
  const { registeredName, registeredPhone, theme, setTheme } = useAppStore();
  ...
  const { data: walletStats } = useWalletStats(userId);
      ...
      <div className="container mx-auto px-4 py-4 space-y-4 pb-24">
        <div className="bg-white dark:bg-surface-800 rounded-2xl shadow-sm">
          <div className="bg-gradient-to-r from-surface-900 to-surface-800 px-5 pt-6 pb-12 rounded-t-2xl">
            <div className="flex items-start justify-between">
              <div className="w-16 h-16 bg-white/20 rounded-2xl flex items-center justify-center overflow-hidden">
                {user?.id ? (
                  <img 
                    src={`https://ui-avatars.com/api/?name=${encodeURIComponent(profileData.name || user?.first_name || 'U')}&background=C9A24D&color=fff&size=64&bold=true`}
                    alt=""
          ...
          <div className="-mt-6 mx-4 bg-white dark:bg-surface-700 rounded-2xl shadow-md px-4 py-3 grid grid-cols-3 text-center divide-x divide-gray-100 dark:divide-gray-600 relative z-10">
            ...
            <div>
              <p className="text-lg font-bold text-amber-600 dark:text-amber-400">{walletStats?.wallet?.balance ?? 0}</p>
              ...
              <div className="flex items-center gap-3">
                ...
                <div className="text-left">
                  <span className="text-sm font-medium text-surface-900 dark:text-white">
                    {language === 'ru' ? 'Пригласить друзей' : "Do'stlarni taklif qilish"}
                  </span>
                  <p className="text-xs text-surface-500 dark:text-surface-400">
                    {language === 'ru' ? `+2 монеты каждому` : 'Har biriga +2 tanga'}
                  </p>
                </div>
              </div>
        ...
        <div className="bg-white dark:bg-surface-800 rounded-2xl shadow-sm overflow-hidden">
          ...
          >
            <div className="flex items-center gap-3">
              ...
              <span className="text-sm font-medium text-surface-900 dark:text-white">
                {language === 'ru' ? 'Кошелёк (ShopCoin)' : 'Hamyon (ShopCoin)'}
              </span>
            </div>
            <ChevronRight className="w-4 h-4 text-surface-400" />
          </button>
