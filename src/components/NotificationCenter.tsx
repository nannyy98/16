    ...
    <div className="space-y-3">
      {notifications.length === 0 ? (
        <div className="text-center py-12">
          ...
          <p className="text-sm text-surface-500">{language === 'ru' ? 'Нет уведомлений' : "Bildirishnomalar yo'q"}</p>
        </div>
      ) : (
        <>
          {unreadCount > 0 && (
            <div className="flex items-center justify-between">
              <span className="text-xs text-surface-500">{unreadCount} {language === 'ru' ? 'непрочитанных' : "o'qilmagan"}</span>
              <button onClick={() => markAllRead.mutate()} className="text-xs text-surface-900 font-medium hover:underline">
                {language === 'ru' ? 'Прочитать все' : "Hammasini o'qish"}
              </button>
            </div>
          )}
